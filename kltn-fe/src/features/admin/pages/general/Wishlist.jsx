const WishList = () => (
    <div className="main-content">
                {/* <!-- Page Title Start --> */}
                <div className="row">
                    <div className="colxl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div className="page-title-wrapper">
                            <div className="page-title-box">
                                <h4 className="page-title">Wishlist</h4>
                            </div>
                            <div className="breadcrumb-list">
                                <ul>
                                    <li className="breadcrumb-link">
                                        <a href="index.html"><i className="fas fa-home mr-2"></i>Dashboard</a>
                                    </li>
                                    <li className="breadcrumb-link active">Wishlist</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                {/* <!-- Table Start --> */}
                <div className="row">
                    <div className="col-lg-12">
						<div className="card">
							<div className="card-header">
								<h5>Wishlist</h5>
							</div>
							<div className="card-body pb-2">
								<div className="row">
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box">                                 
										  <div className="media">
											<img src="../../assets/admin/images/product.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Dining Chair</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box">                                 
										  <div className="media">
											<img src="../../assets/admin/images/product2.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Ceiling Light</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box">                                 
										  <div className="media">
											<img src="../../assets/admin/images/product3.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Wooden Sofa</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box">                                 
										  <div className="media">
											<img src="../../assets/admin/images/product4.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Dining Chair</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Dining Chair</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product2.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Ceiling Light</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product3.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Wooden Sofa</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product4.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Dining Chair</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Dining Chair</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product2.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Ceiling Light</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product3.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Wooden Sofa</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
									<div className="col-xl-3 col-lg-6 col-md-6">
										<div className="prooduct-details-box ad-details-box2">
										  <div className="media">
											<img src="../../assets/admin/images/product4.png" alt=""/>
											<div className="media-body ms-3">
											  <div className="product-name">
												<h6><a href="javascript:;" title="">Dining Chair</a></h6>
											  </div>
											  <div className="rating"><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i><i className="fa fa-star"></i></div>
											  <div className="price"> 
												<div className="text-muted me-2">Price: 210$</div>
											  </div>
											  <div className="avaiabilty">
												<div className="text-success">In stock</div>
												<button type="button" className="btn btn-primary squer-btn sm-btn mt-2 mr-2">Move to Cart</button>
											  </div>
											</div>
										  </div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
                </div>

				<div className="ad-footer-btm">
					<p>Copyright 2022 © SplashDash All Rights Reserved.</p>
				</div>  
			</div>  
  );
  
  export default WishList;